...
public class ActorDaoHibernateImpl implements ActorDao {
    ...
    public Set<Film> getFilmByActorName(String actorName) {
        String hql = "FROM ActorEntity a INNER JOIN FilmActor fa INNER JOIN FilmEntity f WHERE a.first_name = :actorName";
        Session session = sessionFactory.getCurrentSession();
        Query<ActorEntity> query = session.createQuery(hql);
        query.setParameter("first_name", actorName);
        return query.list().stream().map(FilmMapper::fromEntity).collect(Collectors.toSet());
    }
}
